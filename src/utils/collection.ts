...
export function groupByDone(
	...
	// 구현하세요.
	//
	const groupTrue = tasks.filter((task) => task.done === true)
	const groupFalse = tasks.filter((task) => task.done === false)

	return { true: groupTrue, false: groupFalse }
}
